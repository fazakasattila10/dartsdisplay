...
const PLAYER_NAME_HISTORY_KEY = 'teamcricket.playerNameHistory';
...
function freshPlayer(name: string, _isVirtual = false, wins = 0): PlayerState {
  return {
...
function normalizeTeamPlayerNames(names: string[]) {
  const fallbacks = ['PL.1 - A Team', 'PL.1 - B Team', 'PL.2 - A Team', 'PL.2 - B Team'];
  return fallbacks.map((fallback, idx) => (names[idx] || fallback).trim() || fallback) as [string, string, string, string];
}

function canWin(players: PlayerState[], idx: number) {
  const teamIdx = teamIndexForPlayer(idx);
...
function applyCricketTurn(players: PlayerState[], playerIdx: number, turnCounts: Record<string, number>) {
...
export default function KrikettTeamsScreen({ onExit, clubId, initialBoardNr, onOpenScoring, onOpenDisplay, onOpenDisplayById, openNewGameRequestKey, replayRequestKey }: Props) {  useDisplayWakeLock(true);
  const { width, height } = useWindowDimensions();
  const insets = useSafeAreaInsets();
  const isPortrait = height >= width;
  const [playerNameHistory, setPlayerNameHistory] = useState<string[]>(() => loadPlayerNameHistory());
  const [players, setPlayers] = useState<PlayerState[]>(() => [freshPlayer('PL.1 - A Team'), freshPlayer('PL.1 - B Team'), freshPlayer('PL.2 - A Team'), freshPlayer('PL.2 - B Team')]);
...
const pushToFirebase = useCallback(async () => {
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (showSetupDialog || showBoardDialog || showWinnerDialog) return;
  ...
  }, [round, active, gameStarter, showSetupDialog, showBoardDialog, showWinnerDialog, roundFlashOpacity, roundFlashScale]);
  ...
  const restartAfterWin = (winnerIdx: number) => {
    ...
    const winnerTeam = teamIndexForPlayer(winnerIdx);
    const nextPlayers = players.map((p, idx) =>
      freshPlayer(p.name, false, p.wins + (teamIndexForPlayer(idx) === winnerTeam ? 1 : 0))
    );
  ...
  const finishTurn = (nextPlayers: PlayerState[], playerIdx: number) => {
    ...
    if (canWin(nextPlayers, playerIdx)) {
      const teamIdx = teamIndexForPlayer(playerIdx);
      setWinnerText(`${teamIdx === 0 ? 'A Team' : 'B Team'} wins!`);
  ...
  const onEnter = () => {
    ...
    const nextPlayers = applyCricketTurn(players, active, pendingTurn);
  ...
  const replayCurrentMatch = () => {
    ...
    setWarnedBusyBoard(null);
    setMenuBoardTmp(null);
    startGame(undefined, boardNr != null, boardNr, replayDraft);
  };
  ...
  const startGame = (_selectedVariant: any, withBoard: boolean, forcedBoard?: number | null, draftOverride?: SetupDraft) => {
    const sourceDraft = draftOverride ?? draft;
    const finalNames = normalizeTeamPlayerNames(sourceDraft.humanNames);
    const nextPlayers = finalNames.map((name) => freshPlayer(name));
      ...
      <View style={styles.screen}>
        ...
        <View style={styles.tableWrap}>
          {players.map((player, idx) => (
            <View key={player.id} style={[styles.tableRow, idx % 2 === 0 ? styles.tableRowTeamA : styles.tableRowTeamB, idx === active ? styles.tableRowActive : null]}>
              {!isPortrait ? (
        ...
        <View style={styles.modalOverlay}>
          ...
          <View style={styles.modalCardWide}>
            ...
            {draft.humanNames.map((name, idx) => (
              <View key={`human-${idx}`} style={[styles.setupFieldWrap, idx > 0 ? { marginTop: 10 } : null]}>
                <TextInput
                  ...
                  placeholder={[ 'Pl.1 - A Team', 'Pl.1 - B Team', 'Pl.2 - A Team', 'Pl.2 - B Team' ][idx]}
            ...
            <View style={styles.modalBtns}>
              <Pressable style={[styles.modalBtn, styles.modalBtnOk]} onPress={() => requestStart()}>
                <Text style={styles.modalBtnTextOk}>Team Cricket</Text>
              </Pressable>
            </View>
...
cricketMarkChipLabel: {
  ...
  teamTotalsPortraitWrap: { alignItems: 'center', justifyContent: 'center', marginTop: 8 },
  teamTotalsPortraitText: { fontSize: 22, fontWeight: '900', color: 'rgba(61,255,47,0.82)' },
