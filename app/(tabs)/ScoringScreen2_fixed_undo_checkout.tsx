...
export default function ScoringScreen2({ onExit, clubId, initialBoardNr }: Props) {
  ...
  const undoStack = useRef<Array<{ players: [PlayerStats, PlayerStats]; active: 0 | 1; lastThrow: [number, number] }>>([]);
  ...
  const pushUndo = () => {
    ...
    undoStack.current.push({ players: cloned, active: activeRef.current, lastThrow: [...lastThrowRef.current] as [number, number] });
  };
  ...
  const onUndo = () => {
    if (!canInteract || isOnline || matchConfig.vsVirtual) return;
    ...
    <View style={styles.safe}>
      ...
      <Modal visible={showBoardDialog} transparent animationType="fade" onRequestClose={() => setShowBoardDialog(false)}>
        ...
        <View style={styles.modalOverlay}><View style={styles.modalCard}>
          ...
          <View style={styles.namesBlockNoPad}>
            <TextInput
              value={draft.player1}
              ...
              onFocus={() => { if (!name1Touched) { setDraft((prev) => ({ ...prev, player1: '' })); setName1Touched(true); } }}
            />
            <View style={styles.namesGap} />
            {draft.vsVirtual ? (
              <View style={styles.virtualPickerWrap}>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.virtualPickerRow}>
                  {Array.from({ length: 12 }).map((_, idx) => {
                    const level = idx + 1;
                    const selected = draft.virtualLevel === level;
                    return (
                      <Pressable key={level} onPress={() => setDraft((prev) => ({ ...prev, virtualLevel: level }))} style={[styles.levelChip, selected ? styles.levelChipOn : null]}>
                        <Text style={[styles.levelChipText, selected ? styles.levelChipTextOn : null]}>{`Lv.${level}`}</Text>
                      </Pressable>
                    );
                  })}
                </ScrollView>
              ...
              <TextInput
                ...
                placeholderTextColor="rgba(0,0,0,0.45)"
                onFocus={() => { if (!name2Touched) { setDraft((prev) => ({ ...prev, player2: '' })); setName2Touched(true); } }}
              />
            )}
          </View>
          ...
          <View style={styles.setupRowBetween}>
            ...
            <Switch
              ...
              onValueChange={(value) => setDraft((prev) => ({ ...prev, vsVirtual: value, virtualLevel: prev.virtualLevel || 5 }))}
            />
          </View>

          {draft.vsVirtual ? (
            <View style={styles.virtualOrderPreview}>
              <Text style={styles.modalLabel}>Current order</Text>
              <View style={styles.orderRow}><Text style={styles.orderSide}>Left</Text><Text style={styles.orderValue}>{draft.virtualSide === 0 ? `Lv.${draft.virtualLevel}` : draft.player1 || 'Player'}</Text></View>
              <View style={styles.orderRow}><Text style={styles.orderSide}>Right</Text><Text style={styles.orderValue}>{draft.virtualSide === 1 ? `Lv.${draft.virtualLevel}` : draft.player1 || 'Player'}</Text></View>
              <Pressable onPress={toggleDraftVirtualSwap} style={styles.swapInlineBtn}><Text style={styles.swapInlineBtnText}>Swap order</Text></Pressable>
            </View>
          ) : (
            <Pressable onPress={swapHumanNames} hitSlop={8} style={styles.swapInlineBtn}><Text style={styles.swapInlineBtnText}>Swap players</Text></Pressable>
          )}
      ...
      <Modal visible={showMenuDialog2} transparent animationType="fade" onRequestClose={() => setShowMenuDialog2(false)}>
        <View style={styles.modalOverlay}><View style={styles.modalCard}>
          ...
          {draft.vsVirtual ? (
            <View style={styles.virtualOrderPreview}>
              <Text style={styles.modalLabel}>Player order</Text>
              <View style={styles.orderRow}><Text style={styles.orderSide}>Left</Text><Text style={styles.orderValue}>{draft.virtualSide === 0 ? `Lv.${draft.virtualLevel}` : draft.player1 || 'Player'}</Text></View>
              <View style={styles.orderRow}><Text style={styles.orderSide}>Right</Text><Text style={styles.orderValue}>{draft.virtualSide === 1 ? `Lv.${draft.virtualLevel}` : draft.player1 || 'Player'}</Text></View>
              <Pressable onPress={toggleDraftVirtualSwap} hitSlop={8} style={styles.swapInlineBtn}><Text style={styles.swapInlineBtnText}>Swap order</Text></Pressable>
            </View>
          ) : null}
          ...
          {!!clubId?.trim() ? (
            <>
              ...
              <Text style={styles.modalHintSmall}>Club: {clubId} • Current: {menuBoardTmp ?? '-'}</Text>
            </>
          ) : null}
...
const styles = StyleSheet.create({
  ...
  namesGap: { height: 10 },
  namesBlockNoPad: { position: 'relative' },
  ...
  virtualPickerWrap: { marginTop: 0 },
  virtualPickerRow: { gap: 8, paddingVertical: 4 },
  levelChip: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999, backgroundColor: 'rgba(0,0,0,0.08)' },
  levelChipOn: { backgroundColor: 'rgba(47,111,24,0.16)', borderWidth: 1, borderColor: '#2f6f18' },
  levelChipText: { color: 'rgba(0,0,0,0.72)', fontWeight: '900' },
  levelChipTextOn: { color: '#2f6f18' },
  virtualOrderPreview: { marginTop: 10, padding: 10, borderRadius: 10, backgroundColor: 'rgba(0,0,0,0.04)' },
  orderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 4 },
  orderSide: { color: 'rgba(0,0,0,0.52)', fontWeight: '800' },
  orderValue: { color: 'rgba(0,0,0,0.82)', fontWeight: '900' },
