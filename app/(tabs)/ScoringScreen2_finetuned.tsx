...
type UndoEntry = {
  ...
  hadLegStarted: boolean;
  legStarter: 0 | 1;
  virtualPlan: VirtualPlanState;
};

type PendingVirtualCheckoutState = {
...
export default function ScoringScreen2({ onExit, clubId, initialBoardNr }: Props) {
  ...
  const [pendingVirtualCheckout, setPendingVirtualCheckout] = useState<PendingVirtualCheckoutState | null>(null);
  ...
  const pushUndo = () => {
    ...
    undoStack.current.push({
      ...
      hadLegStarted: hasLegStartedRef.current,
      legStarter: legStarterRef.current,
      virtualPlan: cloneVirtualPlan(virtualPlanRef.current),
    });
  };
  const restoreUndoEntry = (entry: UndoEntry) => {
    if (virtualTimeoutRef.current) {
      clearTimeout(virtualTimeoutRef.current);
      virtualTimeoutRef.current = null;
    }
    playersRef.current = entry.players;
    lastThrowRef.current = entry.lastThrow;
    virtualPlanRef.current = cloneVirtualPlan(entry.virtualPlan);
    setPlayers(entry.players);
    setActive(entry.active);
    setLastThrow(entry.lastThrow);
    setHasLegStarted(entry.hadLegStarted);
    setLegStarter(entry.legStarter);
  ...
  const onDigit = (d: number) => canInteract && safeSetInputWith180Limit(`${input}${d}`);
  const onDel = () => canInteract && input && setInput(input.slice(0, -1));
  ...
  const onQuick = (v: number) => canInteract && applyScoring({ kind: 'score', value: v });
  const onUndo = () => {
    if (isOnline) return;
    if (!canInteract && !matchConfig.vsVirtual && !pendingVirtualCheckout) return;
  ...
  useEffect(() => {
    if (!matchConfig.vsVirtual || active !== matchConfig.virtualSide || pendingVirtualCheckout) return;
  ...
  }, [active, matchConfig, pendingVirtualCheckout]);
  ...
  const startGameFromSetup = (withBoard: boolean) => {
    const selectedBoard = withBoard ? menuBoardTmp : null;
  ...
  const handleBoardPickInSetup = (n: number) => {
    ...
    setWarnedBusyBoard(null);
    setMenuBoardTmp((prev) => (prev === n ? null : n));
  };
  ...
  return (
    <View style={styles.safe}>
      <View style={styles.screen}>
        ...
        <Pressable style={styles.cornerCheck} onPress={() => canInteract && applyScoring({ kind: 'check' })}>
      ...
      <Modal visible={showBoardDialog} transparent animationType="fade" onRequestClose={() => setShowBoardDialog(false)}>
        ...
        <View style={styles.modalOverlay}><View style={styles.modalCard}>
          ...
          <View style={styles.setupRowBetween}>
            ...
            <Switch
              ...
              onValueChange={(value) =>
                setDraft((prev) => ({
                  ...prev,
                  vsVirtual: value,
                  virtualLevel: prev.virtualLevel || 5,
                  virtualSide: value ? prev.virtualSide : 1,
                }))
              }
            />
          </View>
          ...
          <View style={styles.namesBlock}>
            {draft.vsVirtual ? (
              <>
                {draft.virtualSide === 0 ? (
                  <>
                    <Pressable onPress={() => setShowLevelPicker(true)} style={styles.modalInputPicker}>
                      <Text style={styles.modalInputPickerText}>{`Lv.${draft.virtualLevel}`}</Text>
                      <Text style={styles.modalInputPickerArrow}>▾</Text>
                    </Pressable>
                ...
                <Pressable onPress={toggleDraftVirtualSwap} hitSlop={10} style={styles.swapBtnBetween}>
          ...
          <View style={styles.modalBtns}>
            <Pressable style={[styles.modalBtn, styles.modalBtnGhost]} onPress={() => setShowMenuDialog1(false)}><Text style={styles.modalBtnTextGhost}>Cancel</Text></Pressable>
            <Pressable style={[styles.modalBtn, styles.modalBtnOk]} onPress={applySetupStep1}><Text style={styles.modalBtnTextOk}>OK</Text></Pressable>
          </View>

          <Pressable onPress={onExit} hitSlop={8} style={styles.exitInline}><Text style={styles.exitInlineText}>Exit scoring mode</Text></Pressable>
        </View></View>
      </Modal>

      <Modal visible={showLevelPicker} transparent animationType="fade" onRequestClose={() => setShowLevelPicker(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.levelPickerCard}>
            <Text style={styles.modalTitle}>Virtual player level</Text>
            <ScrollView style={styles.levelPickerList} showsVerticalScrollIndicator={false}>
                ...
                return (
                  <Pressable
                    ...
                    style={({ pressed }) => [
                      ...
                      selected ? styles.levelPickerItemOn : null,
                      pressed ? styles.levelPickerItemPressed : null,
                    ]}
                  >
                    <Text style={[styles.levelPickerItemText, selected ? styles.levelPickerItemTextOn : null]}>
                      {`Lv.${level}`}
                    </Text>
                  </Pressable>
                );
            ...
            <Pressable style={[styles.modalBtn, styles.modalBtnGhost, { marginTop: 12 }]} onPress={() => setShowLevelPicker(false)}>
      ...
      <Modal visible={showMenuDialog2} transparent animationType="fade" onRequestClose={() => setShowMenuDialog2(false)}>
        <View style={styles.modalOverlay}><View style={styles.modalCard}>
          ...
          <View style={styles.modalBtns}>
            <Pressable style={[styles.modalBtn, styles.modalBtnGhost]} onPress={() => setShowMenuDialog2(false)}><Text style={styles.modalBtnTextGhost}>Cancel</Text></Pressable>
            <Pressable style={[styles.modalBtn, styles.modalBtnGhost]} onPress={() => startGameFromSetup(false)}><Text style={styles.modalBtnTextGhost}>Start without board</Text></Pressable>
            <Pressable style={[styles.modalBtn, styles.modalBtnOk]} onPress={() => startGameFromSetup(true)}><Text style={styles.modalBtnTextOk}>Start Game</Text></Pressable>
          </View>
        </View></View>
      </Modal>
      ...
      <Modal visible={showNamesDialog} transparent animationType="fade" onRequestClose={() => setShowNamesDialog(false)}>
        <View style={styles.modalOverlay}><View style={styles.modalCard}>
          ...
          <TextInput value={draft.player1} onChangeText={(v) => setDraft((prev) => ({ ...prev, player1: v }))} style={styles.modalInput} placeholder="Player 1" placeholderTextColor="rgba(0,0,0,0.45)" />
          <View style={styles.namesGap} />
          {matchConfig.vsVirtual ? (
            <View style={styles.modalInputDisabled}><Text style={styles.modalInputDisabledText}>{`Lv.${matchConfig.virtualLevel}`}</Text></View>
          ) : (
            <TextInput value={draft.player2} onChangeText={(v) => setDraft((prev) => ({ ...prev, player2: v }))} style={styles.modalInput} placeholder="Player 2" placeholderTextColor="rgba(0,0,0,0.45)" />
          )}
          {!matchConfig.vsVirtual ? <Pressable onPress={swapHumanNames} hitSlop={8} style={styles.swapInlineBtn}><Text style={styles.swapInlineBtnText}>Swap players</Text></Pressable> : null}
          <View style={styles.modalBtns}>
            <Pressable style={[styles.modalBtn, styles.modalBtnGhost]} onPress={() => setShowNamesDialog(false)}><Text style={styles.modalBtnTextGhost}>Cancel</Text></Pressable>
            <Pressable style={[styles.modalBtn, styles.modalBtnOk]} onPress={confirmNamesOnly}><Text style={styles.modalBtnTextOk}>OK</Text></Pressable>
          </View>
        </View></View>
      </Modal>
      ...
      <Modal visible={!!pendingVirtualCheckout} transparent animationType="fade" onRequestClose={() => {}}>
        <View style={styles.modalOverlay}><View style={styles.modalCard}>
          <Text style={styles.modalTitle}>Virtual player checkout</Text>
          <Text style={styles.modalLabel}>A virtuális játékos kiszállt. Mehet a következő leg, vagy visszalépsz undóval?</Text>
          <View style={styles.modalBtns}>
            <Pressable style={[styles.modalBtn, styles.modalBtnGhost]} onPress={onUndo}>
              <Text style={styles.modalBtnTextGhost}>UNDO</Text>
            </Pressable>
            <Pressable
              style={[styles.modalBtn, styles.modalBtnOk]}
              onPress={() => {
                if (pendingVirtualCheckout) {
                  finalizeNewLeg(pendingVirtualCheckout.nextPlayers, pendingVirtualCheckout.nextLegStarter);
                }
              }}
            >
      ...
      <Modal visible={showOnlineDetectedDialog} transparent animationType="fade" onRequestClose={() => setShowOnlineDetectedDialog(false)}>
        <View style={styles.modalOverlay}><View style={styles.modalCard}>
          ...
          <Text style={styles.modalLabel}>Úgy tűnik még egy klubtársad játszik ezen a táblán. Online módban vagytok. Ha nem szándékos, akkor válassz másik pályát!</Text>
      ...
      <Modal visible={showInactiveDialog} transparent animationType="fade" onRequestClose={() => setShowInactiveDialog(false)}>
        <View style={styles.modalOverlay}><View style={styles.modalCard}>
          ...
          <Text style={styles.modalLabel}>Inaktivitás miatt lecsatlakoztattunk a boardról. Ha mégis használni szeretnéd, akkor csatlakozz újra.</Text>
...
function KeyFlex(props: { label: string; onPress: () => void; kind?: 'num' | 'action' | 'quick' }) {
  ...
  return <Pressable onPress={props.onPress} style={({ pressed }) => [styles.keyFlex, kind === 'action' ? styles.keyAction : null, kind === 'quick' ? styles.keyQuick : null, pressed ? { opacity: 0.78 } : null]}><Text style={[styles.keyText, kind !== 'num' ? styles.keyTextAction : null]}>{props.label}</Text></Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  setupRowBetween: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 14, marginBottom: 4 },
  switchLabel: { fontWeight: '900', color: 'rgba(0,0,0,0.72)' },
  modalInputPicker: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1, borderColor: 'rgba(0,0,0,0.2)', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 12, backgroundColor: '#ffffff' },
  modalInputPickerText: { color: 'rgba(0,0,0,0.85)', fontWeight: '800' },
  modalInputPickerArrow: { color: 'rgba(0,0,0,0.55)', fontWeight: '900', fontSize: 16 },
  ...
  levelPickerCard: { width: '86%', maxWidth: 320, maxHeight: '70%', backgroundColor: '#fff', borderRadius: 14, padding: 14 },
  levelPickerList: { marginTop: 4 },
  levelPickerItem: { borderRadius: 10, paddingVertical: 12, paddingHorizontal: 12, backgroundColor: 'rgba(0,0,0,0.05)', marginBottom: 8 },
  levelPickerItemOn: { backgroundColor: 'rgba(47,111,24,0.16)', borderWidth: 1, borderColor: '#2f6f18' },
  levelPickerItemPressed: { opacity: 0.8 },
  levelPickerItemText: { color: 'rgba(0,0,0,0.78)', fontWeight: '900' },
  levelPickerItemTextOn: { color: '#2f6f18' },
  swapInlineBtn: { marginTop: 8, alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999, backgroundColor: 'rgba(0,0,0,0.08)' },
  swapInlineBtnText: { color: 'rgba(0,0,0,0.72)', fontWeight: '900' },
